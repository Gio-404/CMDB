...
def collect():
    ...
    data['manfacturer'] = raw_data['Manufacturer']
...
def get_os_info():
    distriutor = subprocess.Popen("lsb_release -a|grep 'Distributor ID'",
                                  stdout=subprocess.PIPE, shell=True)
    distriutor = distriutor.stdout.read().decode().split(":")
    release = subprocess.Popen("lsb_release -a|grep 'Description'",
                               stdout=subprocess.PIPE, shell=True)

    release = release.stdout.read().decode().split(":")
    data_dic = {
        "os_distribution": distriutor[1].strip() if len(distriutor) > 1 else "",
...
def get_ram_info():
    ...
    for line in raw_list:
        if line.startswith("Memeory Device"):
    ...
    for item in raw_ram_list:
        ...
        for i in item:
            ...
            if len(data) == 2:
                key, v = data
                if key == 'Size':
                    if v.strip() != "No Module Install":
                        ram_item_to_dic['capacity'] = v.spilt()[0].strip()
                        item_ram_size = round(v.spilt()[0])
                    else:
...
def get_nic_info():
    raw_data = subprocess.Popen("inconfig -a", stdout=subprocess.PIPE, shell=True)
